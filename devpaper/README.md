...
**路径易错点**：当前目录已在 **`devpaper/`** 内时，请用 **`node src/cli.mjs`** 或 **`npm run html:day -- …`**；**不要**再写 `node devpaper/src/cli.mjs`（会变成 `devpaper/devpaper/…` 报错）。在**仓库根**时，用 **`node devpaper/src/cli.mjs`** 或根目录的 **`npm run dp:day -- …`** / **`npm run devpaper -- …`**。
...
| 是什么 | 路径 |
|--------|------|
| **手记月历**（格子状态、链到 `dist`） | **`devpaper/hub/index.html`** |
...
2. **只要月历、不要 Node 控制台**：仓库根 **`npx --yes serve .`**，在终端给出的地址后加 **`/devpaper/hub/index.html`**。  
...
**生成结果**：目录 **`devpaper/dist/2026-04/`**（目录名即 **`YYYY-MM`**）
...
1. 安装 Node **≥ 18**。  
2. 在 **`devpaper/`** 目录执行 **`npm install`**（安装 `marked`；只需做一次）。  
3. 手记源文件写在 **`devpaper/logs/YYYY-MM-DD.md`**（同一天可多篇，每篇一个 `##` 标题）。
...
**在仓库根**（与 `devpaper` 同级）执行：

```bash
npm run dp:hub
```

**在只进入了 `devpaper` 目录时**执行：
...
- **`http://localhost:xxxx/devpaper/hub/index.html`**
...
1. **放进项目**：本包在 `devpaper/`；根目录可有 [`.cursor/rules/devpaper-log.mdc`](../.cursor/rules/devpaper-log.mdc) 引导何时写手记；**完整格式骨架**见 [docs/log-authoring-guide.md](docs/log-authoring-guide.md)（随仓库 / npm 包分发；英文规范，正文语言不限）。  
2. **日志**：`devpaper/logs/*.md`，由 Agent 按 [Log authoring guide](docs/log-authoring-guide.md) / 自己维护。  
...
**前置条件**：Node.js **≥ 18**；在 **`devpaper` 目录**执行 `npm install` 一次。

**路径约定**：未指定 `--logs` / `--out` 时，CLI 固定使用**本包根目录**下的 `logs/` 与 `dist/`（即 `devpaper/logs`、`devpaper/dist`），与当前工作目录无关。你传入的相对路径仍相对**当前 shell 所在目录**解析。
...
| 短命令（在 devpaper 目录） | `npm run html:day -- …` 等 | 见本包 `package.json` | `./logs` | `./dist` | `html:day` / `html:day:bs` / `html:day:reader` / `html:day:reader-night` / `html:all` / `html:month` / `html:week`、`idx`、`hub` |
...
在 `devpaper` 目录执行 `npm link` 后，可将 `node src/cli.mjs` 换为全局命令 **`devpaper`**。
...
- **自动化测试**：在 `devpaper/` 下执行 **`npm test`**（仓库根 **`npm test`**）。
...
月刊产物在 **`devpaper/dist/2026-04/index.html`**（目录名为 **`YYYY-MM`**）。需要手记月历或页内一键生成时，见上文 **「日历在哪…」** 与 **第四节**（推荐 **`npm run dp:hub`**）。
...
**不覆盖真实 `logs/` 的一页演示**：在**仓库根**执行 **`npm run dp:demo`**，输出到 `devpaper/.demo-out/<YYYY-MM>/` 与 `devpaper/.demo-logs/`（见 [CHANGELOG.md](CHANGELOG.md) 与根 [README.md](../README.md)）。
