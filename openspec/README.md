...
| 能力 ID | 主要实现 |
|---------|-----------|
| `daily-log` | `devpaper/src/parse.mjs`、手记 Markdown 约定 |
| `newspaper-html` | `devpaper/src/build.mjs`、`templates/`、`assets/`、`hub/` |
| `ai-memory-index` | `devpaper/src/index-build.mjs`、`index` / `hub` 子命令 |
| `cursor-skill-devpaper` | Cursor **Rule**（`.cursor/rules/devpaper-log.mdc`）+ **`devpaper/docs/log-authoring-guide.md`**（npm 包内唯一 `docs` 文件） |
...
6. **实现**：在 `devpaper/` 改代码、补测、更新 `devpaper/README.md` / `CHANGELOG.md` 等。
...
- [ ] CLI 帮助文案（`devpaper/src/cli.mjs`）是否与规格中的子命令/参数一致？
